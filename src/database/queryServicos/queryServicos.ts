      ...
      type servico = {
        ...
        async function selectByCode( codigo:number ) {
            ...
            if( isNaN(codigo)){
                ...
                const result = await db.getAllAsync(`SELECT * FROM servicos where codigo = ${codigo}`);
           // console.log(result);
            return result;
          }

          async function selectAll() {
                 const result = await db.getAllAsync(`SELECT * FROM servicos`);
            console.log(result);
          }

          
          async function selectByDescription( query:any, limit:number ) {
            const result = await db.getAllAsync(`SELECT * FROM servicos WHERE  aplicacao like ? OR codigo like ? LIMIT ?`, `%${query}%`, `%${query}%`,`${limit}` );
          //  console.log(result);
            return result;
             }
