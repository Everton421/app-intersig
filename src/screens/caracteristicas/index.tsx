    ...
    useFocusEffect(
                ()=>{
                    ...
                    async function busca(){ 
                            let data:any  = await useQueryCaracterisca.selectAll()
                            if( data?.length > 0  ){
                                setDados(data) 
                            }   
                        }
  ...
  if( !caracteristicaSelecionada?.descricao ) return Alert.alert("Erro!", "É necessario informar a descrição para poder gravar!") 
    ...
    try{
        ...
        if(result.status === 200 ){
            try{
              let resultDb = await useQueryCaracterisca.update(objCaracteristica, caracteristicaSelecionada.codigo);
                }catch(e){
            return Alert.alert('Erro!', 'Erro ao Tentar registrar característica no banco local!');
            }
            setVisible(false)
